document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            formData.append("subject", "Nowe zapytanie kontaktowe: " + name);
            ...
            .then(async (response) => {
                if (response.status === 200) {
                    btn.innerHTML = '<i data-lucide="check-circle" style="width:18px;height:18px;margin-right:8px;vertical-align:middle;"></i> Pomyślnie wysłano!';
                    btn.style.background = 'var(--success)';
                    btn.style.boxShadow = '0 4px 15px rgba(16, 185, 129, 0.4)';
                    lucide.createIcons();
                    contactForm.reset();
                } else {
                    btn.innerHTML = 'Błąd! Spróbuj ponownie.';
                }
            })
            .catch(error => {
                console.error(error);
